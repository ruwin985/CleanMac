...
const functionName = "cleanmac-license";
...
Deno.serve(async (request: Request): Promise<Response> => {
  try {
    ...
    if (request.method === "GET" && (path === "/" || path === "/health")) {
      return jsonResponse({ ok: true, service: "cleanmac-license", runtime: "supabase-edge" });
    }
    if (request.method === "POST" && path === "/webhooks/paddle") {
      return await handlePaddleWebhook(request, env);
    }
    if (request.method === "POST" && path === "/webhooks/xorpay") {
      return await handleXorPayWebhook(request, env);
    }
    if (request.method === "POST" && path === "/payments/xorpay/orders") {
      return await handleXorPayCreateOrder(request, env);
    }
    if (request.method === "GET" && path.startsWith("/payments/xorpay/orders/")) {
      return await handleXorPayOrderStatus(path, env);
    }
    if (request.method === "POST" && path === "/licenses/activate") {
      return await handleLicenseActivation(request, env);
    }
    if (request.method === "POST" && path === "/licenses/verify") {
...
function envFromDeno(): Env {
  return {
    SUPABASE_URL: requiredEnv("SUPABASE_URL", "CLEANMAC_SUPABASE_URL"),
    SUPABASE_SERVICE_ROLE_KEY: requiredEnv("SUPABASE_SERVICE_ROLE_KEY", "CLEANMAC_SUPABASE_SERVICE_ROLE_KEY"),
    PADDLE_WEBHOOK_SECRET: Deno.env.get("PADDLE_WEBHOOK_SECRET") ?? "",
    PADDLE_API_KEY: Deno.env.get("PADDLE_API_KEY"),
    PADDLE_API_BASE_URL: Deno.env.get("PADDLE_API_BASE_URL"),
    ...
    LICENSE_SIGNING_KEY: requiredEnv("LICENSE_SIGNING_KEY"),
    LICENSE_ENCRYPTION_KEY: Deno.env.get("LICENSE_ENCRYPTION_KEY"),
    LICENSE_HASH_PEPPER: Deno.env.get("LICENSE_HASH_PEPPER"),
    SERVER_TOKEN_SECRET: requiredEnv("SERVER_TOKEN_SECRET"),
    ALLOWED_PRICE_IDS: Deno.env.get("ALLOWED_PRICE_IDS"),
    ALLOWED_PRODUCT_IDS: Deno.env.get("ALLOWED_PRODUCT_IDS"),
    LICENSE_MAX_DEVICES: Deno.env.get("LICENSE_MAX_DEVICES"),
    LICENSE_DEVICE_PRICE_MINOR: Deno.env.get("LICENSE_DEVICE_PRICE_MINOR"),
    APP_NAME: Deno.env.get("APP_NAME"),
    SUPPORT_EMAIL: Deno.env.get("SUPPORT_EMAIL"),
    PADDLE_SIGNATURE_TOLERANCE_SECONDS: Deno.env.get("PADDLE_SIGNATURE_TOLERANCE_SECONDS"),
    XORPAY_AID: Deno.env.get("XORPAY_AID"),
    XORPAY_APP_SECRET: Deno.env.get("XORPAY_APP_SECRET"),
    XORPAY_API_BASE_URL: Deno.env.get("XORPAY_API_BASE_URL"),
    XORPAY_NOTIFY_URL: Deno.env.get("XORPAY_NOTIFY_URL"),
    XORPAY_PRICE: Deno.env.get("XORPAY_PRICE"),
    XORPAY_PAY_TYPE: Deno.env.get("XORPAY_PAY_TYPE")
  };
}
...
function requiredEnv(name: string, fallbackName?: string): string {
  ...
  if (!value) throw new Error(`Missing ${fallbackName ? `${name} or ${fallbackName}` : name}`);
  return value;
}
